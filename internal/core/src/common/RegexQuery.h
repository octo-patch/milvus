...
// Replaces [\s\S] (single Unicode char in RE2 UTF-8 mode) with a
...
adapt_regex_for_boost_utf8(const std::string& pattern) {
    ...
    // The needle is the literal 5-char string: [ \s \S ]
...
// For invalid UTF-8 bytes, returns 1 to skip one byte at a time,
// following SQL LIKE semantics where wildcards match any byte sequence
// regardless of encoding validity.
inline size_t
Utf8WildcardCharByteLen(const char* str, size_t remaining) {
    if (remaining == 0) {
        return 0;
    }
    return Utf8ValidatedCharByteLen(str, remaining);
}
